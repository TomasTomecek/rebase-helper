    ...
    def test_fill_dictionary(self):
        expected_dict = {'added': ['/usr/sbin/test',
                         ...
                         'renamed': ['/usr/lib/libtest3.so.3',
                                     '/usr/lib/libtest3.so']}
        plugin_runner.checkers['pkgdiff'].results_dir = TEST_FILES_DIR
        plugin_runner.checkers['pkgdiff'].fill_dictionary(TEST_FILES_DIR,
                                                          old_version='1.0.1',
                                                          new_version='1.0.2')
        assert plugin_runner.checkers['pkgdiff'].results_dict == expected_dict
    ...
    def test_process_xml(self):
        expected_dict = {'added': ['/usr/sbin/test',
                         ...
                         'renamed': ['/usr/lib/libtest3.so.3',
                                     '/usr/lib/libtest3.so']}
        plugin_runner.checkers['pkgdiff'].results_dir = TEST_FILES_DIR
        res_dict = plugin_runner.checkers['pkgdiff'].process_xml_results(TEST_FILES_DIR,
                                                                         old_version='1.0.1',
                                                                         new_version='1.0.2')
